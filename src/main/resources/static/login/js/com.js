...
var loginform=document.querySelector("#login-form");loginform.setAttribute("novalidate",true);document.addEventListener("blur",function(event){var error=haveError(event.target);if(error){showError(event.target,error);return}removeError(event.target)},true);var haveError=function(field){var v=field.validity;if(v.valid)return;if(v.valueMissing&&field.id=="username")return"必填项！请输入用户名！";if(v.valueMissing&&field.id=="password")return"必填项！请输入密码！";return"请仔细检查！"};var showError=function(field,error){field.classList.add("valid-error");var mark=field.id||field.name;if(!mark)return;var message=field.form.querySelector(".message-error#error-for-"+mark);if(!message){message=document.createElement("div");message.className="message-error";message.id="error-for-"+mark;field.parentNode.insertBefore(message,field.nextSibling)}field.setAttribute("aria-described-by","error-for-"+mark);message.innerHTML=error;message.style.display="block";message.style.vilibility="vilible"};var removeError=function(field){field.classList.remove("valid-error");field.removeAttribute("aria-described-by");var mark=field.id||field.name;if(!mark)return;var message=field.form.querySelector(".message-error#error-for-"+mark);if(!message)return;message.innerHTML="";message.style.display="none";message.style.visibility="hidden"};